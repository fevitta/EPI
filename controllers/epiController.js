...
exports.getById = (req, res, next) => {
  ...
  let shouldGetAttachment = new String(req.query.GetAttachment);
  ...
  if (id.length > 6) {
    ...
    (async () => {
      const browser = await puppeteer.launch({
        ...
        userDataDir: '%LOCALAPPDATA%\Chromium\User Data\Profile 1'
      })
      const page = await browser.newPage()

      //Permitir o download de arquivos e selecionar pasta de destino
      await page._client.send('Page.setDownloadBehavior', { behavior: 'allow', downloadPath: 'C:\Users\felip\Downloads' })
      ...
      if (erroCA == 0) {
        ...
        if (shouldGetAttachment == 'true') {
          console.log('Buscando anexos')
          //https://stackoverflow.com/questions/62790413/set-browser-profile-settings-via-puppeteer
          await page.waitForSelector('#PlaceHolderConteudo_btnVisualizarCA')
          await page.click('#PlaceHolderConteudo_btnVisualizarCA')
          await page.waitForSelector('#PlaceHolderConteudo_btnVisualizarCA')
